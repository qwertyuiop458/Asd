...
function main() {
  ...
  ensureDir(path.join(OUTPUT_DIR, 'direct_images'));
  ensureDir(path.join(OUTPUT_DIR, 'embedded_images'));
  ensureDir(path.join(OUTPUT_DIR, 'raw_data'));
  ...
  for (const entry of entries) {
    ...
    if (['png', 'gif', 'jpg', 'jpeg', 'bmp'].includes(ext)) {
      ...
      fs.writeFileSync(path.join(OUTPUT_DIR, 'direct_images', safeName), data);
    ...
    for (let i = 0; i < pngs.length; i++) {
      ...
      const outPath = path.join(OUTPUT_DIR, 'embedded_images', `${safeName}_png_${i}_offset_${pngs[i].offset}.png`);
      fs.writeFileSync(outPath, pngs[i].data);
    ...
    for (let i = 0; i < gifs.length; i++) {
      const safeName = name.replace(/[\/\\]/g, '_').replace(/\.[^.]+$/, '');
      const outPath = path.join(OUTPUT_DIR, 'embedded_images', `${safeName}_gif_${i}_offset_${gifs[i].offset}.gif`);
      fs.writeFileSync(outPath, gifs[i].data);
      console.log(`    -> EMBEDDED GIF at offset ${gifs[i].offset}`);
      embeddedImages++;
    }
    
    // Save potentially interesting raw data files
    if (!['class', 'mf', 'sf', 'rsa', 'png', 'gif', 'jpg'].includes(ext) && data.length > 50) {
      const safeName = name.replace(/[\/\\]/g, '_');
      fs.writeFileSync(path.join(OUTPUT_DIR, 'raw_data', safeName), data);
    }
  }
  ...
  console.log(`Found ${jarPngs.length} PNG signatures in JAR binary`);
  console.log(`Found ${jarGifs.length} GIF signatures in JAR binary`);
  
  for (let i = 0; i < jarPngs.length; i++) {
    const outPath = path.join(OUTPUT_DIR, 'embedded_images', `jar_deep_png_${i}_offset_${jarPngs[i].offset}.png`);
    fs.writeFileSync(outPath, jarPngs[i].data);
    console.log(`  Saved PNG #${i} (${jarPngs[i].data.length} bytes)`);
  }
  
  for (let i = 0; i < jarGifs.length; i++) {
    const outPath = path.join(OUTPUT_DIR, 'embedded_images', `jar_deep_gif_${i}_offset_${jarGifs[i].offset}.gif`);
    fs.writeFileSync(outPath, jarGifs[i].data);
    console.log(`  Saved GIF #${i} (${jarGifs[i].data.length} bytes)`);
  }
